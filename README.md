...
Copy `deploy/helm/kontor/secrets.example.yaml` to `secrets.yaml` and fill in all
`REPLACE_ME` values. Create the hosted-provider Secret out-of-band so the API
key is not retained in Helm release history. The production overlay contains
