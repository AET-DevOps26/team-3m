...
def test_resolve_prefers_logos_when_key_and_model_set() -> None:
    settings = Settings(logos_api_key="lg-key", logos_embedding_model="text-embed")
    provider = resolve_embedding_provider(settings)
    assert provider is not None
    assert provider.model == "text-embed"


def test_resolve_falls_back_to_ollama_without_logos() -> None:
    settings = Settings(
        logos_api_key="", local_llm_base_url="http://x:11434/v1", local_embedding_model="nomic-embed-text"
    )
    provider = resolve_embedding_provider(settings)
    assert provider is not None
...
def test_resolve_skips_logos_when_embedding_model_missing() -> None:
    settings = Settings(logos_api_key="lg-key", logos_embedding_model="", local_llm_base_url="")
